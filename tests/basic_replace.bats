...
@test "replaces multiple occurrences in one file" {
    ...
    run "$FINDIR" --no-color --danger -y "foo" "bar" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "replaces across multiple files" {
    ...
    run "$FINDIR" --no-color --danger -y "hello" "hi" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "replaces in nested directory files" {
    ...
    run "$FINDIR" --no-color --danger -y "old" "new" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "does not modify files without matches" {
    ...
    run "$FINDIR" --no-color --danger -y "foo" "replaced" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "replaces with empty string (deletion)" {
    ...
    run "$FINDIR" --no-color --danger -y "remove_this_part " "" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "treats search as literal string not regex" {
    ...
    run "$FINDIR" --no-color --danger -y "\$10.00" "\$20.00" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "handles special regex characters in search" {
    ...
    run "$FINDIR" --no-color --danger -y "[this]" "[that]" "$TEST_DIR"
    [ "$status" -eq 0 ]
