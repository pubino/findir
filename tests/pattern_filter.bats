...
@test "pattern filter only processes matching files" {
    ...
    run "$FINDIR" --no-color --danger -p "*.py" "hello" "goodbye" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "pattern filter with -p flag" {
    ...
    run "$FINDIR" --no-color --danger -p "*.txt" "hello" "goodbye" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "pattern filter with --pattern flag" {
    ...
    run "$FINDIR" --no-color --danger --pattern "*.js" "hello" "goodbye" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "pattern with question mark wildcard" {
    ...
    run "$FINDIR" --no-color --danger -p "*.p?" "hello" "goodbye" "$TEST_DIR"
    [ "$status" -eq 0 ]
