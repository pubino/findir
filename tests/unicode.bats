...
@test "replaces unicode characters" {
    ...
    run "$FINDIR" --no-color --danger -y "café" "coffee" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "replaces CJK characters" {
    ...
    run "$FINDIR" --no-color --danger -y "世界" "world" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "replaces emoji characters" {
    ...
    run "$FINDIR" --no-color --danger -y "🍕" "🍔" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "replaces accented characters" {
    ...
    run "$FINDIR" --no-color --danger -y "naïve" "sophisticated" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "replaces with unicode in replacement string" {
    ...
    run "$FINDIR" --no-color --danger -y "world" "мир" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "handles mixed unicode and ASCII" {
    ...
    run "$FINDIR" --no-color --danger -y "αβγ" "delta" "$TEST_DIR"
    [ "$status" -eq 0 ]
...
@test "preserves unicode in non-matching parts" {
    ...
    run "$FINDIR" --no-color --danger -y "search" "found" "$TEST_DIR"
    [ "$status" -eq 0 ]
